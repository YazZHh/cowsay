...
do
    ...
    ascii_val=$(printf "%d" "'$char")           # On la convertit en sa valeur dans la table ascii

    val=$(expr $ascii_val + $decalage)          # Ensuite on la décale de la valeur de la clé fournie
    ...
    encrypted_msg=$(echo "$encrypted_msg$encrypted_val")    # On ajoute le caractère converti à la chaine de caractères finale
